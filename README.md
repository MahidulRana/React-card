...
<details>
<summary> File sharing with Samba </summary>
First, create a Test File in gnb1. by Comand: 

```bash
docker exec -it gnb1 sh
# Now in gnb1 container's shell
echo "Hello from gnb1" > /tmp/gnb1-file.txt
exit
```
Then, install smbclient in gnb1:
...
exit
```
Thirdly, Upload the File to Samba. <br>
From gnb1, upload the file to the gnb1share share:

```bash
docker exec -it gnb1 sh
smbclient //samba.share/gnb1share -U user%password -c 'put /tmp/gnb1-file.txt gnb1-file.txt'
exit
```
When this completes, the file gnb1-file.txt is now on the Samba share (in /mount/gnb1 inside the Samba container, and likely in ./shared/gnb1 on your host if that’s how you mounted it).

Then, Install smbclient in ue1:

```bash
docker exec -it ue1 sh
apt-get update && apt-get install -y smbclient
exit
```
Now, Download the File on ue1:

```bash
docker exec -it ue1 sh
smbclient //samba.share/gnb1share -U user%password -c 'get gnb1-file.txt /tmp/from-gnb1.txt'
exit
```
Last, Confirm the Contents:

```bash
docker exec -it ue1 cat /tmp/from-gnb1.txt
```
If you see “Hello from gnb1” congratulations: you’ve successfully transferred a file from gnb1 to ue1 via the Samba share!

## gNB1 to ue1 file transfer via samba:

![Samba](https://github.com/MobileComputingWiSe24-25/mobcom-teammblcp/blob/main/scpmodel-c-team-mobcomp/Testing_Results/gnb1%20to%20ue1%20file%20transfer%20via%20samba.png)
